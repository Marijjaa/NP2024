...
class Scheduler<T>{
    ...
    public void add(Date date, T t) {
        scheduler.putIfAbsent(date, t);
    }

    public boolean remove(Date date) {
        return (boolean) scheduler.remove(date);
    }
    public T next(){
    ...
    public ArrayList<T> getAll(Date b, Date e) {
        NavigableMap<Date, T> subMap = scheduler.subMap(b, true, e, true);
        ArrayList<T> list = new ArrayList<>(subMap.values());
        return list;
    }

...
class Scheduler<T> {
    ...
    public Timestamp<T> next(){
        return list.stream().filter(i -> i.getTime().isAfter(LocalDateTime.now())).min((t1, t2) -> t1.compareTo(t2)).get();
    }
    public Timestamp<T> last(){
        return list.stream().filter(i -> i.getTime().isBefore(LocalDateTime.now())).max((t1, t2) -> t1.compareTo(t2)).get();
    }
    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end){
        return list.stream().filter(i -> i.getTime().isBefore(end) && i.getTime().isAfter(begin)).collect(Collectors.toList());
    }
}

...
class Rule<IN, OUT>{
    ...
    Rule(Predicate<IN> predicate, Function<IN, OUT> function) {
...
public class RuleTester {
    public static void main(String[] args) {
        ...
        if (testCase == 1) { //Test for String,Integer
            ...
            /*
            //Add a rule where if the string contains the string "NP", the result would be index of the first occurrence of the string "NP"
            * */
            rules.add(new Rule<>(
                    str -> str.contains("NP"),
                    str -> str.indexOf("NP")
            ...
            /*
            //Add a rule where if the string starts with the string "NP", the result would be length of the string
            * */
            rules.add(new Rule<>(
            ...
            // Add a rule where if the student has at least 3 grades, the result would be the max grade of the student
            rules.add(new Rule<>(
                    student -> student.grades.size()>=3,
                    student -> student.grades.stream().max(Comparator.comparingInt(Integer::intValue)).get().doubleValue()
            ));

            // Add a rule where if the student has an ID that starts with 20, the result would be the average grade of the student
            //If the student doesn't have any grades, the average is 5.0
            rules.add(new Rule<>(
                    student -> student.getId().startsWith("20"),
                    student -> student.grades.stream().mapToInt(Integer::intValue).average().orElse(5.0)
            ));
